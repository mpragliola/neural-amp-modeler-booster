...
function boostModelNode(node, gain, path) {
    ...
    const headScaleMatchesLastWeight = typeof headScale === "number" && Math.abs(headScale - lastWeight) < 1e-6;
    if (headScaleMatchesLastWeight) {
        weights[weights.length - 1] = lastWeight * gain;
        config.head_scale = headScale * gain;
...
function walkAndBoost(value, gain, path, reports) {
 ...
 */
export function boostNamProfile(data, boostDb) {
...
export function boostNamFileText(fileContents, boostDb) {
